...
1. **Do Not Infer Logic or Schemas**: Always inspect `backend/models.py`, `backend/lookup_engine.py`, and `PROJECT_SPEC.md` before adding or modifying API endpoints or lookup functions.
...
2. **Short Handle Stems**: Prioritize single-token short stems (`len <= 10`) for versioned candidate generation (`.v2`, `.v3`, `_09`).
3. **Exclude Bare Surnames**: Do not include bare single-token surnames in general multi-keyword handle searches to prevent search query dilution.
